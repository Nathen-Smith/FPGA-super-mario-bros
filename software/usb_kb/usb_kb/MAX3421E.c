...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE buffer[2] = {reg + 2, val};
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, buffer, 0, NULL, 0);
	if (return_code < 0) {
		printf("error");
	}
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, buffer, 0, NULL, 0);
	if (return_code < 0) {
		printf("error");
	}
	return data + nbytes;
